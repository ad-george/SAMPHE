...
export class ReportService {
  async getReport(sessionId: string) {
    const session = await prisma.attendanceSession.findUnique({
      where: { id: sessionId },
      include: {
        unit: { include: { programme: true, studyYear: true, semester: true, department: true } },
    ...
    const reportData = allStudents.map((s: any) => ({
      ...
      submissionTime: session.records.find((r: any) => r.studentId === s.id)?.submissionTime || null,
    }));
  ...
  async generatePDF(sessionId: string) {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      doc.fontSize(10).text(`${uni.address || ''} | ${uni.email || ''} | ${uni.phone || ''}`);
      doc.moveDown();

      doc.fontSize(14).text(`Attendance Report`, { underline: true });
      doc.fontSize(10);
      doc.text(`Faculty: ${session.unit.department.faculty?.name || 'N/A'}`);
      doc.text(`Department: ${session.unit.department.name}`);
      doc.text(`Programme: ${session.unit.programme.name}`);
      ...
      doc.text(`Duration: ${session.attendanceDuration} minutes`);
      doc.moveDown();

      doc.text(`Total: ${stats.total} | Present: ${stats.present} | Absent: ${stats.absent} | Rate: ${stats.percentage}%`);
      doc.moveDown();
      ...
      students.forEach((s: any) => {
        ...
        doc.text(s.registrationNumber, 50, y);
        doc.text(s.fullName, 150, y);
        doc.text(s.status, 400, y);
        doc.text(s.submissionTime ? new Date(s.submissionTime).toLocaleTimeString() : '-', 450, y);
        doc.moveDown(0.5);
      });
  ...
  async generateExcel(sessionId: string) {
    ...
    worksheet.getCell('A1').value = `${session.university.name} - Attendance Report`;
    worksheet.getCell('A1').font = { bold: true, size: 16 };

    worksheet.getCell('A3').value = `Unit: ${session.unit.code} - ${session.unit.name}`;
    worksheet.getCell('A4').value = `Lecturer: ${session.lecturer.fullName}`;
    worksheet.getCell('A5').value = `Date: ${session.sessionDate.toDateString()}`;
