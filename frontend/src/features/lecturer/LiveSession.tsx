...
const LiveSession = () => {
  ...
  const copyLink = () => {
    // Use the current hostname (ngrok URL or IP)
  ...
  // No active session — show meaningful content
  if (!session) {
    return (
      <div className="space-y-6 max-w-6xl mx-auto">
        <div className="bg-slate-700 border border-slate-600 rounded-2xl p-8 shadow-sm text-center">
          <div className="w-16 h-16 rounded-full bg-slate-600 flex items-center justify-center mx-auto mb-4 border border-slate-500">
          ...
          <div className="flex items-center justify-center gap-3">
            <button
              ...
              className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-500 transition shadow-sm"
            >
              Start New Session
            </button>
            <button
              onClick={() => navigate("/lecturer/history")}
              className="px-6 py-2.5 bg-slate-600 border border-slate-500 text-white rounded-xl text-sm font-medium hover:bg-slate-500 transition"
            >
              View History
            </button>
          </div>
            ...
            <div className="space-y-3">
              {recentHistory.map((h: any, i: number) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex items-center justify-between p-4 bg-slate-600 rounded-xl border border-slate-500"
                  >
                    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm">
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-700 border border-slate-600 rounded-2xl p-6 shadow-sm">
        <div>
        ...
        <div className="flex items-center gap-3">
          <div className="bg-slate-600 border border-slate-500 rounded-xl px-5 py-3 text-center min-w-[100px]">
          ...
          <button
            onClick={copyLink}
            className="px-4 py-2.5 bg-slate-600 text-emerald-400 border border-slate-500 rounded-lg text-sm font-medium hover:bg-slate-500 transition"
          >
            Copy Link
          </button>
          <button
            onClick={end}
            className="px-4 py-2.5 bg-rose-900/20 text-rose-400 border border-rose-800 rounded-lg text-sm font-medium hover:bg-rose-900/40 transition"
          >
            End Session
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm text-center">
          <p className="text-xs text-slate-400 uppercase">Total</p>
          <p className="text-2xl font-bold text-white mt-1">{totalStudents}</p>
        </div>
        <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm text-center">
          <p className="text-xs text-slate-400 uppercase">Present</p>
          <p className="text-2xl font-bold text-emerald-400 mt-1">{present}</p>
        </div>
        <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm text-center">
          <p className="text-xs text-slate-400 uppercase">Absent</p>
          <p className="text-2xl font-bold text-rose-400 mt-1">{absent}</p>
        </div>
        <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm text-center">
          <p className="text-xs text-slate-400 uppercase">Rate</p>
          <p className="text-2xl font-bold text-blue-400 mt-1">{rate}%</p>
        </div>
      </div>

      {outside > 0 && (
        <div className="bg-amber-900/20 border border-amber-800 rounded-xl p-4 text-sm text-amber-400 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm">
            ...
            <div className="space-y-2">
              {recentCheckins.map((r: any, i: number) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-2.5 bg-slate-600 rounded-lg border border-slate-500"
                >
          ...
          {/* Share Link */}
          <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-white mb-2">Share Link</h3>
            <div className="bg-slate-600 rounded-lg p-3 border border-slate-500 flex items-center justify-between gap-2">
              <code className="text-xs text-emerald-400 truncate">{`${window.location.origin}/attendance/${session?.token}`}</code>
              <button
                onClick={copyLink}
                className="text-xs px-3 py-1.5 bg-emerald-600 text-white rounded-md hover:bg-emerald-500 transition shrink-0"
              >
                Copy
              </button>
            </div>
          ...
          {/* Session Info */}
          <div className="bg-slate-700 border border-slate-600 rounded-2xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-white mb-3">Session Info</h3>
        ...
        <div className="lg:col-span-2 bg-slate-700 border border-slate-600 rounded-2xl overflow-hidden shadow-sm">
          <div className="p-4 border-b border-slate-600 flex items-center justify-between">
            <h3 className="font-bold text-white">Real-time Attendance Feed</h3>
                  ...
                  <tr>
                    <td colSpan={5} className="p-16 text-center text-slate-400">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p>No check-ins yet. Share the link to begin.</p>
                      </div>
                    </td>
                  </tr>
                )}
