...
const HodLayout = () => {
  ...
  const navItems = [
    ...
    { path: "/hod/lecturers", label: "Lecturers", icon: "◊" },
    { path: "/hod/students", label: "Students", icon: "◎" },
    { path: "/hod/programmes", label: "Programs", icon: "▣" },
    { path: "/hod/units", label: "Units", icon: "◉" },
    { path: "/hod/academic-structure", label: "Academic Structure", icon: "◫" },
    { path: "/hod/class-records", label: "Attendance Records", icon: "⊞" },
    { path: "/hod/reports", label: "Reports", icon: "◧" },
    { path: "/hod/analytics", label: "Analytics", icon: "◯" },
  ];

  useEffect(() => {
    if (!user || user.role !== "HOD") navigate("/login");
    fetchNotifications();
    fetchProfile();
    api
      .get("/hod/me")
      .then((r) => {
        if (r.data.data?.department?.name)
          setDeptName(r.data.data.department.name);
      })
      .catch(() => {});
  }, [user, navigate]);
  ...
  const handleSearch = async (q: string) => {
    ...
    try {
      const res = await api.get(`/hod/search?q=${encodeURIComponent(q)}`);
  ...
  const goToResult = (type: string) => {
    setShowSearch(false);
    setSearchQ("");
    if (type === "lecturer") navigate("/hod/lecturers");
    if (type === "student") navigate("/hod/students");
    if (type === "unit") navigate("/hod/units");
    if (type === "program") navigate("/hod/programmes");
  };
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.put("/hod/me", payload);
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <form onSubmit={saveProfile} className="space-y-4">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-16 h-16 rounded-full bg-slate-200 flex items-center justify-center text-xl font-bold text-slate-600 border border-slate-300 overflow-hidden">
                  {profileForm.avatar ? (
                    <img
                      src={profileForm.avatar}
                      className="w-full h-full object-cover"
              ...
              <div className="border-t border-slate-100 pt-4">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 mb-3 focus:border-emerald-400 focus:outline-none"
                />
                <input
